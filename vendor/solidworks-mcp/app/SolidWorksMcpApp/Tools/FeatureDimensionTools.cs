...
public class FeatureDimensionTools(StaDispatcher sta, IFeatureDimensionService featureDimensions, IFeatureCacheManager cacheManager)
{
    ...
    [McpServerTool, Description("Intelligently finds a specific dimension token (e.g., 'D7@Edge Line...') based on its physical type ('length', 'angle', 'radius', or 'diameter') rather than guessing the string.")]
